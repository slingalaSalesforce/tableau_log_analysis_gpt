...
def fine_tune(client, file_id):
    ...
    response = client.fine_tuning.jobs.create(
        ...
        hyperparameters={
            "n_epochs": 4,                # Recommended: 3-8 epochs depending on size
            "batch_size": "auto",         # Let OpenAI decide or manually set (like 8, 16)
            "learning_rate_multiplier": 0.1  # Fine-tuning: 0.02-0.2 depending on model drift risk
        }
    )
    print(f'Created fine-tune job ID: {response}')
    return response.id
